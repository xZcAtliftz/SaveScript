...
local function createSafeTable()
    local function createMeta(parentKey)
        return {
            __index = function(t, k)
                if k == "Value" and parentKey ~= "Value" then
                    return false
                end
                t[k] = setmetatable({}, createMeta(k))
                return t[k]
            end,
            __newindex = function(t, k, v)
                rawset(t, k, v ~= nil and v or setmetatable({}, createMeta(k)))
            end
        }
    end
    return setmetatable({}, createMeta(nil))
end

local Options = createSafeTable()
...
function Library:SetConfig(Data, Ignored)
    ...
    if type(Options) ~= "table" then
        return warn("Options table is not defined.")
    end
    
    for Option, Value in pairs(Data) do
        if type(Options[Option]) == "table" and type(Options[Option].SetValue) == "function" and not table.find(Ignored, Option) then
...
do
    ...
    function Funcs:AddDropdown(Idx, Info)
        ...
        if Info.Default then
            if Info.Multi then
                if Dropdown.Values[Info.Default] then
...
function Library:CreateWindow(WindowTitle,MobileMode)
    local Window = {
        Tabs = {};
    };
    ...
    if MobileMode then
        Enabled_Mobiles = true
        Outer = Library:Create('Frame', {
            BackgroundColor3 = Library.OutlineColor;
            BorderSizePixel = 0;
            Position = UDim2.new(0.5, -275, 0.5, -100);
